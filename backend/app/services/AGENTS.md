...
## OVERVIEW
`match_service.py`（排表/开赛/记分）+ `points_service.py`（积分幂等结算/排行榜），编排 `tournaments/` 纯逻辑引擎与 `plugins/` 玩法插件，是 api 层与引擎之间的唯一业务入口。
...
build_schedule_for_competition（比赛进 ongoing 时）
  ...
  → 轮空对局直接 finished / result_type=win（winner=participant_a，不建会话）
...
start_match（裁判开赛）
  ...
  → registry.get(gameplay_plugin).create_session(match.id, config) 建 GameSession
  → match.status=in_progress → manager.broadcast state_update
...
| `start_match` | match_service | 校验裁判归属 → 插件 create_session → 广播 state_update |
| `record_match_result` | match_service | 单败淘汰禁平局 → 引擎 record_result → 落库 |
...
- `start_match` 的 config 里 `"seed": match.id` 作为插件会话种子；`sides` 固定 participant_a=defender / participant_b=attacker。
