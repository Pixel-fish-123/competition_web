...
``min(ceil(log2(n)) + 1, 7)``, but only round 1 is materialized at
construction time (seed order — no results exist yet, so the standings order
degenerates to seed order). Each subsequent round is generated by
``generate_next_round()`` ONLY once every real match of the previous round has
been recorded, and it pairs participants by the CURRENT standings —
...
- E2 轮空: a bye counts as 1 win / 0 net score and is scored automatically in
  ...
  participant receives two while un-byed participants remain; the bye goes to
  the lowest-ranked participant that has not yet had one.
