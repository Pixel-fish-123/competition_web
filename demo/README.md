...
- 防守方用己方占领格（或地图边界）把一片非防守方区域完全围住时触发**包围**：该区域**整体变为防守方地块**（含攻击方格），**可多次触发**；L1 属于区域但本身不被转换。
- 攻击方占领**并激活** L1 源头即「顶端直胜」，直接获胜。
- 时间耗尽则比分高者胜（防 : 攻）。
...
随机开局前必须先导入歌曲库：至少 **23 首且歌名唯一**。接口顺序固定为 `POST /api/songs` 导入，再 `POST /api/init` 开局；未导入歌曲库时 `init` 返回 400。
