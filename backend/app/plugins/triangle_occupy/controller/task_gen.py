...
def _weighted_choice(table: list[dict], key: str = "weight") -> dict:
...
def generate_tasks(seed: int | None = None) -> list[dict]:
    ...
    weights = [1] * len(DIFFICULTY_TABLE)

    chosen: list[dict] = []
    for _ in range(21):
        d = _weighted_choice(
            [{"tier": t, "weight": w} for t, w in zip(tiers, weights)]
        )
        diff = next(x for x in DIFFICULTY_TABLE if x["tier"] == d["tier"])
    ...
    for cell_id in range(21):
        diff = chosen[cell_id]
        task = _weighted_choice(TASK_TABLE)
