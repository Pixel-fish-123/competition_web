...
- L1 挑战：`game.py:166-205`（`_occupy_l1`）。
  ...
  - **攻击方得手：立刻 +1 能量**（含夺回）并重置持有计时基准；满 8 点直接胜利（`_l1_victory`）。
...
### F. 胜利规则
- **L1 能量胜利**：`game.py:78-101`（`_accrue_l1_energy` / `_l1_victory`）——攻击方持有 L1 期间积累能量（占领 +1、每 2 分钟 +1），满 8 点（`L1_ENERGY_TARGET`）直接获胜（`win_type="l1_energy"`），防守方分数保留；防守方夺回后能量保留暂停。旧「激活 L1 秒胜」已移除（`check_l1_energy` 替代 `check_top_victory`，`game.py:476-478`）。
...
**3. 激活**：攻击方的普通格只有与能源格连通才「激活」计分；未激活不计分。L1 占领本身即计分（与激活无关；旧「激活 L1 秒胜」已移除）。
...
**6. 胜利**：攻击方持有 L1 积累能量满 **8 点**（占领 +1、每 2 分钟 +1；防守方夺回后暂停保留）即「L1 能量胜利」；否则时间耗尽按比分定胜负，平局为 draw。
