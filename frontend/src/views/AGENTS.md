...
| CompetitionDetail.vue | 详情：顶部返回键 + 报名/撤销 + admin 审批（通过/拒绝）+ 赛程卡片 + 场次排名（胜/负/平/积分，issue 1/11）+ 名称显示 nickname 优先 |
| MatchPlay.vue | 对局页：WS 实时状态刷新；pending 裁判可「随机选边」（issue 2）；结束比赛 → 内联判定面板（导入玩法日志自动判定 → 人工微调 → 保存结果锁定，issue 13/14）；`result_locked` 后只读 |
| Announcements.vue | 公告列表（标题/时间/附件数，点击进详情） |
| AnnouncementDetail.vue | 公告详情：正文 + 附件下载链接（pdf/word/zip） |
| Profile.vue | 个人中心：改昵称、建队/拉人/移除成员/**解散队伍**（issue 3）、我的报名、积分流水 |
| Rankings.vue | 排行榜：单一全局积分榜（`/points/leaderboard`） |

## ADMIN VIEWS
| 文件 | 职责 |
|------|------|
| AdminLayout.vue | 侧边栏布局 + `<router-view>` 出口（选手/比赛/积分/流量/公告 5 项） |
...
| Announcements.vue | 公告管理：发布（标题/正文/多附件上传）+ 删除（连带附件文件） |
